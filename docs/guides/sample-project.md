...
1.  Create a new file named `main.tf` and copy the following Terraform template
    ...
    resource "confluentcloud_kafka" "test-basic-cluster" {
...
3.  Run the following command to apply the plan and create cloud resources:
    ...
    confluentcloud_kafka.test-basic-cluster: Creating...
    confluentcloud_service_account.test-sa: Creation complete after 1s [id=sa-***] <--- the Service Account's ID
    confluentcloud_kafka.test-basic-cluster: Still creating... [10s elapsed]
    confluentcloud_kafka.test-basic-cluster: Creation complete after 14s [id=lkc-***] <--- the Kafka cluster's ID
...
3.  Append the following resource definitions into `main.tf` and save the file.
    ...
    resource "confluentcloud_kafka_topic" "orders" {
      kafka = confluentcloud_kafka.test-basic-cluster.id
      topic_name = "orders"
      partitions_count = 4
      replication_factor = 3
      http_endpoint = confluentcloud_kafka.test-basic-cluster.http_endpoint
      config = {
...
confluentcloud_kafka.test-basic-cluster: Destroying... [id=lkc-odgpo]
confluentcloud_kafka.test-basic-cluster: Still destroying... [id=lkc-odgpo, 10s elapsed]
confluentcloud_kafka.test-basic-cluster: Still destroying... [id=lkc-odgpo, 20s elapsed]
confluentcloud_kafka.test-basic-cluster: Destruction complete after 23s
